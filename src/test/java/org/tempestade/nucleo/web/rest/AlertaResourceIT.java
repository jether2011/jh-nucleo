...
public class AlertaResourceIT {
    ...
    @Test
    @Transactional
    public void getAllAlertasByOperadorUsuarioIsEqualToSomething() throws Exception {
        // Initialize the database
        alertaRepository.saveAndFlush(alerta);
        Usuario operadorUsuario = UsuarioResourceIT.createEntity(em);
        em.persist(operadorUsuario);
        em.flush();
        alerta.setOperadorUsuario(operadorUsuario);
        alertaRepository.saveAndFlush(alerta);
        Long operadorUsuarioId = operadorUsuario.getId();

        // Get all the alertaList where operadorUsuario equals to operadorUsuarioId
        defaultAlertaShouldBeFound("operadorUsuarioId.equals=" + operadorUsuarioId);

        // Get all the alertaList where operadorUsuario equals to operadorUsuarioId + 1
        defaultAlertaShouldNotBeFound("operadorUsuarioId.equals=" + (operadorUsuarioId + 1));
    }
