...
public class PlanoUsuarioResourceIT {
    ...
    @Test
    @Transactional
    public void getAllPlanoUsuariosByIdIsEqualToSomething() throws Exception {
        // Get already existing entity
        Plano id = planoUsuario.getId();
        planoUsuarioRepository.saveAndFlush(planoUsuario);
        Long idId = id.getId();

        // Get all the planoUsuarioList where id equals to idId
        defaultPlanoUsuarioShouldBeFound("idId.equals=" + idId);

        // Get all the planoUsuarioList where id equals to idId + 1
        defaultPlanoUsuarioShouldNotBeFound("idId.equals=" + (idId + 1));
    }
