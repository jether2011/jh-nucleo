...
public class ContatoAlvoResourceIT {
    ...
    public static ContatoAlvo createEntity(EntityManager em) {
        ...
        // Add required entity
        Alvo alvo;
        if (TestUtil.findAll(em, Alvo.class).isEmpty()) {
            alvo = AlvoResourceIT.createEntity(em);
            em.persist(alvo);
            em.flush();
        } else {
            alvo = TestUtil.findAll(em, Alvo.class).get(0);
        }
    ...
    public static ContatoAlvo createUpdatedEntity(EntityManager em) {
        ...
        Alvo alvo;
        if (TestUtil.findAll(em, Alvo.class).isEmpty()) {
            alvo = AlvoResourceIT.createUpdatedEntity(em);
            em.persist(alvo);
            em.flush();
        } else {
            alvo = TestUtil.findAll(em, Alvo.class).get(0);
    ...
    public void getAllContatoAlvosByContatoIsEqualToSomething() throws Exception {
        ...
        Contato contato = contatoAlvo.getContato();
        contatoAlvoRepository.saveAndFlush(contatoAlvo);
