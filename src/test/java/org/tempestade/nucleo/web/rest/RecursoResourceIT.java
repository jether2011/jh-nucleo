...
public class RecursoResourceIT {
    ...
    public static Recurso createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, RecursoTipo.class).isEmpty()) {
            recursoTipo = RecursoTipoResourceIT.createEntity(em);
            em.persist(recursoTipo);
            em.flush();
        } else {
            recursoTipo = TestUtil.findAll(em, RecursoTipo.class).get(0);
        }
    ...
    public static Recurso createUpdatedEntity(EntityManager em) {
        ...
        RecursoTipo recursoTipo;
        if (TestUtil.findAll(em, RecursoTipo.class).isEmpty()) {
            recursoTipo = RecursoTipoResourceIT.createUpdatedEntity(em);
            em.persist(recursoTipo);
            em.flush();
        } else {
            recursoTipo = TestUtil.findAll(em, RecursoTipo.class).get(0);
    ...
    public void getAllRecursosByRecursoTipoIsEqualToSomething() throws Exception {
        ...
        RecursoTipo recursoTipo = recurso.getRecursoTipo();
        recursoRepository.saveAndFlush(recurso);
