...
import {
  BoletimPrevisaoComponentsPage,
  /* BoletimPrevisaoDeleteDialog, */
  BoletimPrevisaoUpdatePage,
} from './boletim-previsao.page-object';
...
describe('BoletimPrevisao e2e test', () => {
  ...
  /* it('should create and save BoletimPrevisaos', async () => {
        ...
        await promise.all([
            ...
            boletimPrevisaoUpdatePage.ventosOrigemPontosCardeaisSelectLastOption(),
            boletimPrevisaoUpdatePage.ventoRajadaVentoVmFaixaSelectLastOption(),
        ...
        expect(await boletimPrevisaoUpdatePage.getCondicaoTempoInput()).to.eq('condicaoTempo', 'Expected CondicaoTempo value to be equals to condicaoTempo');
        expect(await boletimPrevisaoUpdatePage.getObservacaoInput()).to.eq('observacao', 'Expected Observacao value to be equals to observacao');
        ...
        expect(await boletimPrevisaoUpdatePage.getVentovelocidademediakmhInput()).to.eq('5', 'Expected ventovelocidademediakmh value to be equals to 5');
        expect(await boletimPrevisaoUpdatePage.getVentosObservacaoInput()).to.eq('ventosObservacao', 'Expected VentosObservacao value to be equals to ventosObservacao');
        ...
        expect(await boletimPrevisaoUpdatePage.getTempestadeObservacaoInput()).to.eq('tempestadeObservacao', 'Expected TempestadeObservacao value to be equals to tempestadeObservacao');
        expect(await boletimPrevisaoUpdatePage.getChuvaObservacaoInput()).to.eq('chuvaObservacao', 'Expected ChuvaObservacao value to be equals to chuvaObservacao');
        expect(await boletimPrevisaoUpdatePage.getCreatedInput()).to.contain('2001-01-01T02:30', 'Expected created value to be equals to 2000-12-31');
        expect(await boletimPrevisaoUpdatePage.getUpdatedInput()).to.contain('2001-01-01T02:30', 'Expected updated value to be equals to 2000-12-31');
        ...
        expect(await boletimPrevisaoComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
    }); */
  ...
  /* it('should delete last BoletimPrevisao', async () => {
        ...
        expect(await boletimPrevisaoDeleteDialog.getDialogTitle())
            .to.eq('nucleoApp.boletimPrevisao.delete.question');
