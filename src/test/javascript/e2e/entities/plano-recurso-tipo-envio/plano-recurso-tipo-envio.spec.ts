...
describe('PlanoRecursoTipoEnvio e2e test', () => {
  ...
  /* it('should create and save PlanoRecursoTipoEnvios', async () => {
        ...
        expect(await planoRecursoTipoEnvioUpdatePage.getDescricaoInput()).to.eq('descricao', 'Expected Descricao value to be equals to descricao');
        expect(await planoRecursoTipoEnvioUpdatePage.getQtdInput()).to.eq('5', 'Expected qtd value to be equals to 5');
        expect(await planoRecursoTipoEnvioUpdatePage.getCreatedInput()).to.contain('2001-01-01T02:30', 'Expected created value to be equals to 2000-12-31');
        expect(await planoRecursoTipoEnvioUpdatePage.getUpdatedInput()).to.contain('2001-01-01T02:30', 'Expected updated value to be equals to 2000-12-31');
        ...
        expect(await planoRecursoTipoEnvioComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
    }); */
  ...
  /* it('should delete last PlanoRecursoTipoEnvio', async () => {
        ...
        expect(await planoRecursoTipoEnvioDeleteDialog.getDialogTitle())
            .to.eq('nucleoApp.planoRecursoTipoEnvio.delete.question');
