...
describe('BoletimPrevVariavelMet e2e test', () => {
  ...
  /* it('should create and save BoletimPrevVariavelMets', async () => {
        ...
        expect(await boletimPrevVariavelMetUpdatePage.getDescricaoInput()).to.eq('descricao', 'Expected Descricao value to be equals to descricao');
        expect(await boletimPrevVariavelMetUpdatePage.getCreatedInput()).to.contain('2001-01-01T02:30', 'Expected created value to be equals to 2000-12-31');
        expect(await boletimPrevVariavelMetUpdatePage.getUpdatedInput()).to.contain('2001-01-01T02:30', 'Expected updated value to be equals to 2000-12-31');
        ...
        expect(await boletimPrevVariavelMetComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
    }); */
  ...
  /* it('should delete last BoletimPrevVariavelMet', async () => {
        ...
        expect(await boletimPrevVariavelMetDeleteDialog.getDialogTitle())
            .to.eq('nucleoApp.boletimPrevVariavelMet.delete.question');
