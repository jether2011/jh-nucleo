...
export class PontosCardeaisComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.pontosCardeaisService
      ...
      .subscribe(
        (res: HttpResponse<IPontosCardeais[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInPontosCardeais();
  }
  ...
  registerChangeInPontosCardeais(): void {
    this.eventSubscriber = this.eventManager.subscribe('pontosCardeaisListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IPontosCardeais[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/pontos-cardeais'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
