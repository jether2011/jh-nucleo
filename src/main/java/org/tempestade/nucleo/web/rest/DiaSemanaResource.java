...
public class DiaSemanaResource {
    ...
    private final DiaSemanaQueryService diaSemanaQueryService;

    public DiaSemanaResource(DiaSemanaService diaSemanaService, DiaSemanaQueryService diaSemanaQueryService) {
        this.diaSemanaService = diaSemanaService;
        this.diaSemanaQueryService = diaSemanaQueryService;
    }
    ...
    public ResponseEntity<List<DiaSemanaDTO>> getAllDiaSemanas(DiaSemanaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get DiaSemanas by criteria: {}", criteria);
        Page<DiaSemanaDTO> page = diaSemanaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /dia-semanas/count} : count all the diaSemanas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/dia-semanas/count")
    public ResponseEntity<Long> countDiaSemanas(DiaSemanaCriteria criteria) {
        log.debug("REST request to count DiaSemanas by criteria: {}", criteria);
        return ResponseEntity.ok().body(diaSemanaQueryService.countByCriteria(criteria));
    }
