...
public class AvisoMeteorologicoResource {
    ...
    private final AvisoMeteorologicoQueryService avisoMeteorologicoQueryService;

    public AvisoMeteorologicoResource(AvisoMeteorologicoService avisoMeteorologicoService, AvisoMeteorologicoQueryService avisoMeteorologicoQueryService) {
        this.avisoMeteorologicoService = avisoMeteorologicoService;
        this.avisoMeteorologicoQueryService = avisoMeteorologicoQueryService;
    }
    ...
    public ResponseEntity<List<AvisoMeteorologicoDTO>> getAllAvisoMeteorologicos(AvisoMeteorologicoCriteria criteria, Pageable pageable) {
        log.debug("REST request to get AvisoMeteorologicos by criteria: {}", criteria);
        Page<AvisoMeteorologicoDTO> page = avisoMeteorologicoQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /aviso-meteorologicos/count} : count all the avisoMeteorologicos.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/aviso-meteorologicos/count")
    public ResponseEntity<Long> countAvisoMeteorologicos(AvisoMeteorologicoCriteria criteria) {
        log.debug("REST request to count AvisoMeteorologicos by criteria: {}", criteria);
        return ResponseEntity.ok().body(avisoMeteorologicoQueryService.countByCriteria(criteria));
    }
