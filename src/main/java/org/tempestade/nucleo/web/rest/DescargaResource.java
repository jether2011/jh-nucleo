...
public class DescargaResource {
    ...
    private final DescargaQueryService descargaQueryService;

    public DescargaResource(DescargaService descargaService, DescargaQueryService descargaQueryService) {
        this.descargaService = descargaService;
        this.descargaQueryService = descargaQueryService;
    }
    ...
    public ResponseEntity<List<DescargaDTO>> getAllDescargas(DescargaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Descargas by criteria: {}", criteria);
        Page<DescargaDTO> page = descargaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /descargas/count} : count all the descargas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/descargas/count")
    public ResponseEntity<Long> countDescargas(DescargaCriteria criteria) {
        log.debug("REST request to count Descargas by criteria: {}", criteria);
        return ResponseEntity.ok().body(descargaQueryService.countByCriteria(criteria));
    }
