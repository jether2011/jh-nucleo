...
public class TipoFerramentaResource {
    ...
    private final TipoFerramentaQueryService tipoFerramentaQueryService;

    public TipoFerramentaResource(TipoFerramentaService tipoFerramentaService, TipoFerramentaQueryService tipoFerramentaQueryService) {
        this.tipoFerramentaService = tipoFerramentaService;
        this.tipoFerramentaQueryService = tipoFerramentaQueryService;
    }
    ...
    public ResponseEntity<List<TipoFerramentaDTO>> getAllTipoFerramentas(TipoFerramentaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get TipoFerramentas by criteria: {}", criteria);
        Page<TipoFerramentaDTO> page = tipoFerramentaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /tipo-ferramentas/count} : count all the tipoFerramentas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/tipo-ferramentas/count")
    public ResponseEntity<Long> countTipoFerramentas(TipoFerramentaCriteria criteria) {
        log.debug("REST request to count TipoFerramentas by criteria: {}", criteria);
        return ResponseEntity.ok().body(tipoFerramentaQueryService.countByCriteria(criteria));
    }
