...
public class BoletimPrevObsResource {
    ...
    private final BoletimPrevObsQueryService boletimPrevObsQueryService;

    public BoletimPrevObsResource(BoletimPrevObsService boletimPrevObsService, BoletimPrevObsQueryService boletimPrevObsQueryService) {
        this.boletimPrevObsService = boletimPrevObsService;
        this.boletimPrevObsQueryService = boletimPrevObsQueryService;
    }
    ...
    public ResponseEntity<List<BoletimPrevObsDTO>> getAllBoletimPrevObs(BoletimPrevObsCriteria criteria, Pageable pageable) {
        log.debug("REST request to get BoletimPrevObs by criteria: {}", criteria);
        Page<BoletimPrevObsDTO> page = boletimPrevObsQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /boletim-prev-obs/count} : count all the boletimPrevObs.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/boletim-prev-obs/count")
    public ResponseEntity<Long> countBoletimPrevObs(BoletimPrevObsCriteria criteria) {
        log.debug("REST request to count BoletimPrevObs by criteria: {}", criteria);
        return ResponseEntity.ok().body(boletimPrevObsQueryService.countByCriteria(criteria));
    }
