...
public class MeteogramaResource {
    ...
    private final MeteogramaQueryService meteogramaQueryService;

    public MeteogramaResource(MeteogramaService meteogramaService, MeteogramaQueryService meteogramaQueryService) {
        this.meteogramaService = meteogramaService;
        this.meteogramaQueryService = meteogramaQueryService;
    }
    ...
    public ResponseEntity<List<MeteogramaDTO>> getAllMeteogramas(MeteogramaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Meteogramas by criteria: {}", criteria);
        Page<MeteogramaDTO> page = meteogramaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /meteogramas/count} : count all the meteogramas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/meteogramas/count")
    public ResponseEntity<Long> countMeteogramas(MeteogramaCriteria criteria) {
        log.debug("REST request to count Meteogramas by criteria: {}", criteria);
        return ResponseEntity.ok().body(meteogramaQueryService.countByCriteria(criteria));
    }
