...
public class FerramentaResource {
    ...
    private final FerramentaQueryService ferramentaQueryService;

    public FerramentaResource(FerramentaService ferramentaService, FerramentaQueryService ferramentaQueryService) {
        this.ferramentaService = ferramentaService;
        this.ferramentaQueryService = ferramentaQueryService;
    }
    ...
    public ResponseEntity<List<FerramentaDTO>> getAllFerramentas(FerramentaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Ferramentas by criteria: {}", criteria);
        Page<FerramentaDTO> page = ferramentaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /ferramentas/count} : count all the ferramentas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/ferramentas/count")
    public ResponseEntity<Long> countFerramentas(FerramentaCriteria criteria) {
        log.debug("REST request to count Ferramentas by criteria: {}", criteria);
        return ResponseEntity.ok().body(ferramentaQueryService.countByCriteria(criteria));
    }
