...
public class NotificacaoEnviadaResource {
    ...
    private final NotificacaoEnviadaQueryService notificacaoEnviadaQueryService;

    public NotificacaoEnviadaResource(NotificacaoEnviadaService notificacaoEnviadaService, NotificacaoEnviadaQueryService notificacaoEnviadaQueryService) {
        this.notificacaoEnviadaService = notificacaoEnviadaService;
        this.notificacaoEnviadaQueryService = notificacaoEnviadaQueryService;
    }
    ...
    public ResponseEntity<List<NotificacaoEnviadaDTO>> getAllNotificacaoEnviadas(NotificacaoEnviadaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get NotificacaoEnviadas by criteria: {}", criteria);
        Page<NotificacaoEnviadaDTO> page = notificacaoEnviadaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /notificacao-enviadas/count} : count all the notificacaoEnviadas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/notificacao-enviadas/count")
    public ResponseEntity<Long> countNotificacaoEnviadas(NotificacaoEnviadaCriteria criteria) {
        log.debug("REST request to count NotificacaoEnviadas by criteria: {}", criteria);
        return ResponseEntity.ok().body(notificacaoEnviadaQueryService.countByCriteria(criteria));
    }
