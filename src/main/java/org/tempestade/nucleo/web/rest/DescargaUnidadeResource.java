...
public class DescargaUnidadeResource {
    ...
    private final DescargaUnidadeQueryService descargaUnidadeQueryService;

    public DescargaUnidadeResource(DescargaUnidadeService descargaUnidadeService, DescargaUnidadeQueryService descargaUnidadeQueryService) {
        this.descargaUnidadeService = descargaUnidadeService;
        this.descargaUnidadeQueryService = descargaUnidadeQueryService;
    }
    ...
    public ResponseEntity<List<DescargaUnidadeDTO>> getAllDescargaUnidades(DescargaUnidadeCriteria criteria, Pageable pageable) {
        log.debug("REST request to get DescargaUnidades by criteria: {}", criteria);
        Page<DescargaUnidadeDTO> page = descargaUnidadeQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /descarga-unidades/count} : count all the descargaUnidades.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/descarga-unidades/count")
    public ResponseEntity<Long> countDescargaUnidades(DescargaUnidadeCriteria criteria) {
        log.debug("REST request to count DescargaUnidades by criteria: {}", criteria);
        return ResponseEntity.ok().body(descargaUnidadeQueryService.countByCriteria(criteria));
    }
