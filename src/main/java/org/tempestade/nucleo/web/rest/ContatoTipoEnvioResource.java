...
public class ContatoTipoEnvioResource {
    ...
    private final ContatoTipoEnvioQueryService contatoTipoEnvioQueryService;

    public ContatoTipoEnvioResource(ContatoTipoEnvioService contatoTipoEnvioService, ContatoTipoEnvioQueryService contatoTipoEnvioQueryService) {
        this.contatoTipoEnvioService = contatoTipoEnvioService;
        this.contatoTipoEnvioQueryService = contatoTipoEnvioQueryService;
    }
    ...
    public ResponseEntity<List<ContatoTipoEnvioDTO>> getAllContatoTipoEnvios(ContatoTipoEnvioCriteria criteria, Pageable pageable) {
        log.debug("REST request to get ContatoTipoEnvios by criteria: {}", criteria);
        Page<ContatoTipoEnvioDTO> page = contatoTipoEnvioQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /contato-tipo-envios/count} : count all the contatoTipoEnvios.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/contato-tipo-envios/count")
    public ResponseEntity<Long> countContatoTipoEnvios(ContatoTipoEnvioCriteria criteria) {
        log.debug("REST request to count ContatoTipoEnvios by criteria: {}", criteria);
        return ResponseEntity.ok().body(contatoTipoEnvioQueryService.countByCriteria(criteria));
    }
