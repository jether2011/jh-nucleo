...
public class LayerResource {
     ...
     */
    @GetMapping("/layers")
    public ResponseEntity<List<LayerDTO>> getAllLayers(LayerCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Layers by criteria: {}", criteria);
        Page<LayerDTO> page = layerQueryService.findByCriteria(criteria, pageable);
    ...
    public ResponseEntity<Long> countLayers(LayerCriteria criteria) {
        log.debug("REST request to count Layers by criteria: {}", criteria);
        return ResponseEntity.ok().body(layerQueryService.countByCriteria(criteria));
    }
