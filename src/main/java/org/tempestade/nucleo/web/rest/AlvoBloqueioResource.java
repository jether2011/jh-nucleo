...
public class AlvoBloqueioResource {
    ...
    private final AlvoBloqueioQueryService alvoBloqueioQueryService;

    public AlvoBloqueioResource(AlvoBloqueioService alvoBloqueioService, AlvoBloqueioQueryService alvoBloqueioQueryService) {
        this.alvoBloqueioService = alvoBloqueioService;
        this.alvoBloqueioQueryService = alvoBloqueioQueryService;
    }
    ...
    public ResponseEntity<List<AlvoBloqueioDTO>> getAllAlvoBloqueios(AlvoBloqueioCriteria criteria, Pageable pageable) {
        log.debug("REST request to get AlvoBloqueios by criteria: {}", criteria);
        Page<AlvoBloqueioDTO> page = alvoBloqueioQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /alvo-bloqueios/count} : count all the alvoBloqueios.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/alvo-bloqueios/count")
    public ResponseEntity<Long> countAlvoBloqueios(AlvoBloqueioCriteria criteria) {
        log.debug("REST request to count AlvoBloqueios by criteria: {}", criteria);
        return ResponseEntity.ok().body(alvoBloqueioQueryService.countByCriteria(criteria));
    }
