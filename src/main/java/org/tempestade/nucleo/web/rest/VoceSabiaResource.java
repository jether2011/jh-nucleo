...
public class VoceSabiaResource {
    ...
    private final VoceSabiaQueryService voceSabiaQueryService;

    public VoceSabiaResource(VoceSabiaService voceSabiaService, VoceSabiaQueryService voceSabiaQueryService) {
        this.voceSabiaService = voceSabiaService;
        this.voceSabiaQueryService = voceSabiaQueryService;
    }
    ...
    public ResponseEntity<List<VoceSabiaDTO>> getAllVoceSabias(VoceSabiaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get VoceSabias by criteria: {}", criteria);
        Page<VoceSabiaDTO> page = voceSabiaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /voce-sabias/count} : count all the voceSabias.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/voce-sabias/count")
    public ResponseEntity<Long> countVoceSabias(VoceSabiaCriteria criteria) {
        log.debug("REST request to count VoceSabias by criteria: {}", criteria);
        return ResponseEntity.ok().body(voceSabiaQueryService.countByCriteria(criteria));
    }
