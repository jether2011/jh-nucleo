...
public class RecursoTipoResource {
    ...
    private final RecursoTipoQueryService recursoTipoQueryService;

    public RecursoTipoResource(RecursoTipoService recursoTipoService, RecursoTipoQueryService recursoTipoQueryService) {
        this.recursoTipoService = recursoTipoService;
        this.recursoTipoQueryService = recursoTipoQueryService;
    }
    ...
    public ResponseEntity<List<RecursoTipoDTO>> getAllRecursoTipos(RecursoTipoCriteria criteria, Pageable pageable) {
        log.debug("REST request to get RecursoTipos by criteria: {}", criteria);
        Page<RecursoTipoDTO> page = recursoTipoQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /recurso-tipos/count} : count all the recursoTipos.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/recurso-tipos/count")
    public ResponseEntity<Long> countRecursoTipos(RecursoTipoCriteria criteria) {
        log.debug("REST request to count RecursoTipos by criteria: {}", criteria);
        return ResponseEntity.ok().body(recursoTipoQueryService.countByCriteria(criteria));
    }
