...
public class RedeResource {
    ...
    private final RedeQueryService redeQueryService;

    public RedeResource(RedeService redeService, RedeQueryService redeQueryService) {
        this.redeService = redeService;
        this.redeQueryService = redeQueryService;
    }
     ...
     */
    @GetMapping("/redes")
    public ResponseEntity<List<RedeDTO>> getAllRedes(RedeCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Redes by criteria: {}", criteria);
        Page<RedeDTO> page = redeQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /redes/count} : count all the redes.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/redes/count")
    public ResponseEntity<Long> countRedes(RedeCriteria criteria) {
        log.debug("REST request to count Redes by criteria: {}", criteria);
        return ResponseEntity.ok().body(redeQueryService.countByCriteria(criteria));
    }
