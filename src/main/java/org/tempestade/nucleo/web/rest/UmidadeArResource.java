...
public class UmidadeArResource {
    ...
    private final UmidadeArQueryService umidadeArQueryService;

    public UmidadeArResource(UmidadeArService umidadeArService, UmidadeArQueryService umidadeArQueryService) {
        this.umidadeArService = umidadeArService;
        this.umidadeArQueryService = umidadeArQueryService;
    }
    ...
    public ResponseEntity<List<UmidadeArDTO>> getAllUmidadeArs(UmidadeArCriteria criteria, Pageable pageable) {
        log.debug("REST request to get UmidadeArs by criteria: {}", criteria);
        Page<UmidadeArDTO> page = umidadeArQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /umidade-ars/count} : count all the umidadeArs.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/umidade-ars/count")
    public ResponseEntity<Long> countUmidadeArs(UmidadeArCriteria criteria) {
        log.debug("REST request to count UmidadeArs by criteria: {}", criteria);
        return ResponseEntity.ok().body(umidadeArQueryService.countByCriteria(criteria));
    }
