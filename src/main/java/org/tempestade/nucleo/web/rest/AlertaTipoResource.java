...
public class AlertaTipoResource {
    ...
    private final AlertaTipoQueryService alertaTipoQueryService;

    public AlertaTipoResource(AlertaTipoService alertaTipoService, AlertaTipoQueryService alertaTipoQueryService) {
        this.alertaTipoService = alertaTipoService;
        this.alertaTipoQueryService = alertaTipoQueryService;
    }
    ...
    public ResponseEntity<List<AlertaTipoDTO>> getAllAlertaTipos(AlertaTipoCriteria criteria, Pageable pageable) {
        log.debug("REST request to get AlertaTipos by criteria: {}", criteria);
        Page<AlertaTipoDTO> page = alertaTipoQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /alerta-tipos/count} : count all the alertaTipos.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/alerta-tipos/count")
    public ResponseEntity<Long> countAlertaTipos(AlertaTipoCriteria criteria) {
        log.debug("REST request to count AlertaTipos by criteria: {}", criteria);
        return ResponseEntity.ok().body(alertaTipoQueryService.countByCriteria(criteria));
    }
