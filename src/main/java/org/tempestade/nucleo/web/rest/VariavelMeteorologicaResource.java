...
public class VariavelMeteorologicaResource {
    ...
    private final VariavelMeteorologicaQueryService variavelMeteorologicaQueryService;

    public VariavelMeteorologicaResource(VariavelMeteorologicaService variavelMeteorologicaService, VariavelMeteorologicaQueryService variavelMeteorologicaQueryService) {
        this.variavelMeteorologicaService = variavelMeteorologicaService;
        this.variavelMeteorologicaQueryService = variavelMeteorologicaQueryService;
    }
    ...
    public ResponseEntity<List<VariavelMeteorologicaDTO>> getAllVariavelMeteorologicas(VariavelMeteorologicaCriteria criteria, Pageable pageable) {
        log.debug("REST request to get VariavelMeteorologicas by criteria: {}", criteria);
        Page<VariavelMeteorologicaDTO> page = variavelMeteorologicaQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /variavel-meteorologicas/count} : count all the variavelMeteorologicas.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/variavel-meteorologicas/count")
    public ResponseEntity<Long> countVariavelMeteorologicas(VariavelMeteorologicaCriteria criteria) {
        log.debug("REST request to count VariavelMeteorologicas by criteria: {}", criteria);
        return ResponseEntity.ok().body(variavelMeteorologicaQueryService.countByCriteria(criteria));
    }
