...
public class PlanoStatusResource {
    ...
    private final PlanoStatusQueryService planoStatusQueryService;

    public PlanoStatusResource(PlanoStatusService planoStatusService, PlanoStatusQueryService planoStatusQueryService) {
        this.planoStatusService = planoStatusService;
        this.planoStatusQueryService = planoStatusQueryService;
    }
    ...
    public ResponseEntity<List<PlanoStatusDTO>> getAllPlanoStatuses(PlanoStatusCriteria criteria, Pageable pageable) {
        log.debug("REST request to get PlanoStatuses by criteria: {}", criteria);
        Page<PlanoStatusDTO> page = planoStatusQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /plano-statuses/count} : count all the planoStatuses.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/plano-statuses/count")
    public ResponseEntity<Long> countPlanoStatuses(PlanoStatusCriteria criteria) {
        log.debug("REST request to count PlanoStatuses by criteria: {}", criteria);
        return ResponseEntity.ok().body(planoStatusQueryService.countByCriteria(criteria));
    }
