...
public class ContatoAlvoResource {
    ...
    private final ContatoAlvoQueryService contatoAlvoQueryService;

    public ContatoAlvoResource(ContatoAlvoService contatoAlvoService, ContatoAlvoQueryService contatoAlvoQueryService) {
        this.contatoAlvoService = contatoAlvoService;
        this.contatoAlvoQueryService = contatoAlvoQueryService;
    }
    ...
    public ResponseEntity<List<ContatoAlvoDTO>> getAllContatoAlvos(ContatoAlvoCriteria criteria, Pageable pageable) {
        log.debug("REST request to get ContatoAlvos by criteria: {}", criteria);
        Page<ContatoAlvoDTO> page = contatoAlvoQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /contato-alvos/count} : count all the contatoAlvos.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/contato-alvos/count")
    public ResponseEntity<Long> countContatoAlvos(ContatoAlvoCriteria criteria) {
        log.debug("REST request to count ContatoAlvos by criteria: {}", criteria);
        return ResponseEntity.ok().body(contatoAlvoQueryService.countByCriteria(criteria));
    }
