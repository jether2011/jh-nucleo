...
public class TempestadeProbabilidadeResource {
    ...
    private final TempestadeProbabilidadeQueryService tempestadeProbabilidadeQueryService;

    public TempestadeProbabilidadeResource(TempestadeProbabilidadeService tempestadeProbabilidadeService, TempestadeProbabilidadeQueryService tempestadeProbabilidadeQueryService) {
        this.tempestadeProbabilidadeService = tempestadeProbabilidadeService;
        this.tempestadeProbabilidadeQueryService = tempestadeProbabilidadeQueryService;
    }
    ...
    public ResponseEntity<List<TempestadeProbabilidadeDTO>> getAllTempestadeProbabilidades(TempestadeProbabilidadeCriteria criteria, Pageable pageable) {
        log.debug("REST request to get TempestadeProbabilidades by criteria: {}", criteria);
        Page<TempestadeProbabilidadeDTO> page = tempestadeProbabilidadeQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /tempestade-probabilidades/count} : count all the tempestadeProbabilidades.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/tempestade-probabilidades/count")
    public ResponseEntity<Long> countTempestadeProbabilidades(TempestadeProbabilidadeCriteria criteria) {
        log.debug("REST request to count TempestadeProbabilidades by criteria: {}", criteria);
        return ResponseEntity.ok().body(tempestadeProbabilidadeQueryService.countByCriteria(criteria));
    }
