...
public class BoletimPrevisaoResource {
    ...
    private final BoletimPrevisaoQueryService boletimPrevisaoQueryService;

    public BoletimPrevisaoResource(BoletimPrevisaoService boletimPrevisaoService, BoletimPrevisaoQueryService boletimPrevisaoQueryService) {
        this.boletimPrevisaoService = boletimPrevisaoService;
        this.boletimPrevisaoQueryService = boletimPrevisaoQueryService;
    }
    ...
    public ResponseEntity<List<BoletimPrevisaoDTO>> getAllBoletimPrevisaos(BoletimPrevisaoCriteria criteria, Pageable pageable) {
        log.debug("REST request to get BoletimPrevisaos by criteria: {}", criteria);
        Page<BoletimPrevisaoDTO> page = boletimPrevisaoQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /boletim-previsaos/count} : count all the boletimPrevisaos.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/boletim-previsaos/count")
    public ResponseEntity<Long> countBoletimPrevisaos(BoletimPrevisaoCriteria criteria) {
        log.debug("REST request to count BoletimPrevisaos by criteria: {}", criteria);
        return ResponseEntity.ok().body(boletimPrevisaoQueryService.countByCriteria(criteria));
    }
